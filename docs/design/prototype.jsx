...
import { BookOpen, Play, MessageCircle, CheckCircle, ChevronRight, ChevronLeft, ArrowRight, Sparkles, Brain, Target, Clock, BarChart3, Zap, Send, X, Menu, User, Settings, LogOut, Home, Plus, Search, Star, TrendingUp, AlertCircle, Volume2, Pause, SkipForward, RotateCcw, Award, Flame, Calendar, Eye, FileText, Youtube, Upload, Loader } from "lucide-react";
...
const MOCK_CHAT = [
  { role: "mentor", content: "你好！我看到你正在学习 Karpathy 的 \"Let's build GPT from scratch\" 视频。基于我们的初始评估，你对 Python 编程有不错的基础，但对 Transformer 架构还比较陌生。我建议我们从 Tokenization 开始——这是理解 LLM 的第一块拼图。准备好了吗？" },
];
      ...
      <main className="flex-1 flex items-center justify-center px-6">
        <div className="max-w-2xl text-center">
          ...
          <p className="text-lg text-gray-500 mb-8 max-w-lg mx-auto" style={{ lineHeight: 1.6 }}>
            粘贴一个 YouTube 链接或上传 PDF，Socratiq 会为你生成个性化学习路径，用苏格拉底式引导帮你真正学会。
          </p>
        ...
        <div className="w-full max-w-lg">
          ...
          {step === 0 && (
            <div className="mb-6 p-3 rounded-xl bg-blue-50 border border-blue-100 flex items-start gap-2.5">
              ...
              <div>
                <p className="text-sm font-medium text-blue-800">基于视频内容的自适应评估</p>
                <p className="text-xs text-blue-600 mt-0.5">我已分析了 Karpathy 的视频，从中提取了 5 个核心概念。回答以下问题帮助我了解你的起点，以便跳过你已经会的、聚焦你不会的。</p>
              </div>
            </div>
          )}
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-lg font-semibold text-gray-900 mb-2">正在分析视频内容...</h2>
          ...
          <div className="space-y-3 mt-6 text-left bg-gray-50 rounded-xl p-4">
            {["提取视频字幕", "识别核心概念与前置依赖", "评估难度等级", "准备自适应评估题"].map((s, i) => (
              <div key={i} className="flex items-center gap-2 text-sm">
                {i < 2 ? <CheckCircle className="w-4 h-4 text-green-500" /> : i === 2 ? <Loader className="w-4 h-4 text-blue-500 animate-spin" /> : <div className="w-4 h-4 rounded-full border border-gray-300" />}
                <span className={i < 2 ? "text-gray-500" : i === 2 ? "text-gray-900 font-medium" : "text-gray-400"}>{s}</span>
              </div>
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-xl">
          ...
          <p className="text-sm text-gray-500 mb-8">粘贴 YouTube 链接，开始你的个性化学习之旅</p>
          ...
          <Button size="lg" className="w-full" onClick={handleImport} disabled={!url.trim() || !goal}>
            <Sparkles className="w-4 h-4" /> 生成学习路径
          </Button>

          <div className="mt-6 flex items-center gap-4 justify-center">
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <FileText className="w-3.5 h-3.5" /> PDF 支持即将上线
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <Upload className="w-3.5 h-3.5" /> 更多来源开发中
            </div>
          </div>
        </div>
      </div>
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-start justify-between">
            <div>
              <h1 className="text-lg font-bold text-gray-900">Let's build GPT from scratch</h1>
              <p className="text-sm text-gray-500 mt-0.5">Andrej Karpathy · YouTube</p>
            </div>
...
function VideoLearningPage({ section, onExercise, onBack }) {
  ...
  const sendMessage = async () => {
    ...
    const mentorResponses = [
      ...
      "非常好的分析！你提到了一个关键点。让我们看看视频 23:15 处 Karpathy 的解释，他用了一个非常直观的例子。看完那段后我们继续讨论。",
    ];
          ...
          <div className="bg-gray-900 aspect-video relative flex-shrink-0">
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-white/60 text-xs">Let's build GPT from scratch — Andrej Karpathy</p>
              </div>
            </div>
            {/* Video controls */}
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
        ...
        <Card className="p-4 mb-6" hover onClick={onContinue}>
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-semibold text-gray-900">Let's build GPT from scratch</h3>
              <p className="text-xs text-gray-500">Andrej Karpathy · 5 章节 · 12 概念</p>
...
export default function App() {
  ...
  const showSidebar = ["dashboard", "courses", "explore", "progress"].includes(page);
